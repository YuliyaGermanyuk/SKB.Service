...
{
    class VerificationProtocolTableLook : CertificateTableLook
    {
        /// <summary>
        /// Ширина таблицы.
        /// </summary>
        ...
        internal TableRow CreateTitle(TablesRow NewRow, int TableWidth)
        {
            ...
            {
                TableCell NewTableCell = NewTableRow.AddTableCell(TableWidth / NewRow.ColumnsCount, NewRow.Values[i], -113, -113, JustificationValues.Left, 10,
        ...
        internal TableRow CreateHeader(TablesRow NewRow, int[] ColumnsWidthList)
        {
            ...
            for (int i = 0; i < NewRow.ColumnsCount; i++)
            {
                TableCell NewTableCell = NewTableRow.AddTableCell(ColumnsWidthList[i], NewRow.Values[i], -113, -113, JustificationValues.Center, 10,
        ...
        /// <param name="DeviceTypeName"> Тип прибора (используется тоько для датчика ДП22 для определения предела доспустимой погрешности в зависимости от типа прибора).</param>
        /// <returns></returns>
        public Table GetSensorsTable(WordprocessingDocument ParentDocument, string SensorsNumber, string DeviceTypeName)
        {
            Table table = CalibrationLib.NewTable(ColumnsWidth, 157);
            List<TablesRow> TableRows = GetRows(ParentDocument, SensorsNumber, DeviceTypeName);
        ...
        /// <param name="DeviceTypeName"> Тип прибора (используется тоько для датчика ДП22 для определения предела доспустимой погрешности в зависимости от типа прибора).</param>
        /// <returns></returns>
        public List<TablesRow> GetRows(WordprocessingDocument ParentDocument, string SensorsNumber, string DeviceTypeName)
        {
            Table CurrentTable = ParentDocument == null ? null : ParentDocument.GetTable(ParentTableName);
            ...
            Variables = new string[] { SensorsNumber, Variation.ToString(), ErrorValue.ToString(), RodInfo, MarginOfError };
